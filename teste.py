...
COORDS_6 = [
    ...
    (435, 107), # CONTA (FEITO --)
    (105, 256), # NOVA MORADA (FEITO --)
    (105, 273), # CP4 (FEITO --)
    (177, 273), # CP3 (FEITO --)
    (437, 273), # CONTACTO (FEITO --)
    (150, 321), # NIF ANTIGO (FEITO --)
    (419, 321), # NIF NOVO (FEITO --)
...
COORDS_7 = [
    ...
    (222, 487), # MÓVEL 2 (FEITO)
    (319, 487), # MÓVEL 3 (FEITO)
    (416, 487), # MÓVEL 4 (FEITO)
    ...
    (198, 254), # NÚMERO DE CLIENTE (FEITO)
    (125, 436), # FIXO (FEITO)
    (175,685),   # Dia
    (205,685),  # Mês   
    (230,685),  # Ano
    (80, 685),  # LOCALIDADE (LOCALIDADE DA DATA)(FEITO)
]
...
def preencher_pdf(dados):
    if not os.path.exists(PDF_ORIGINAL):
        ...
        return
    # conseguir os numeros e cvps
    global num_1, cvp_1, num_2, cvp_2, num_3, cvp_3, num_4, cvp_4
    global num1_meo, num2_meo, num3_meo, num4_meo
    global fixo_1, fixo_2, movel_1, movel_2, movel_3, movel_4 # == ALTERAÇAO DE TITULARIDADE ==
    global res_movel_1, res_movel_2, res_movel_3, res_movel_4 # == RESCISÃO DE CONTRATO ==
    ...
    campos_4 = [
    "" if dados["ze_sem_ze"] else ssfid,
    dados["num1_meo"] if dados["num1_meo"] != "" else "",
    dados["num2_meo"] if dados["num2_meo"] != "" else "",
    dados["num3_meo"] if dados["num3_meo"] != "" else "",
    dados["num4_meo"] if dados["num4_meo"] != "" else "",
    dados["num_1"] if dados["num_1"] != "" else "",
    dados["num_2"] if dados["num_2"] != "" else "",
    dados["num_3"] if dados["num_3"] != "" else "",
    dados["num_4"] if dados["num_4"] != "" else "",
    dados["cvp_1"] if dados["cvp_1"] != "" else "",
    dados["cvp_2"] if dados["cvp_2"] != "" else "",
    dados["cvp_3"] if dados["cvp_3"] != "" else "",
    dados["cvp_4"] if dados["cvp_4"] != "" else "",
    dados["nome_completo_movel"],
    dados["nif_movel"],
    "X" if dados["combo_operador"] == "MEO" else "",
    "X" if dados["combo_operador"] == "NOS" else "",
    "X" if dados["combo_operador"] == "NOWO" else "",
    dados["combo_operador"] if dados["combo_operador"] not in ["MEO", "NOS", "NOWO"] else "",
]
    
    # CVP – até 4 números, sem dar erro
    for i in range(4):
        if i < len(linhas_telemoveis) and linhas_telemoveis[i][0].get().strip():
            campos_4.append(dados["cvp_meo"])
        else:
            campos_4.append("")

    # Data
    campos_4.extend([dia, mes, ano])
    ...
    campos_5 = [
        ...
        dados["fixo_pf"],
        dados["cvp_pf"],
        "X" if dados["operador_pf"] == "MEO" else "",
        "X" if dados["operador_pf"] == "NOS" else "",
        "X" if dados["operador_pf"] == "NOWO" else "",
        dados["operador_pf"] if dados["operador_pf"] not in ["MEO", "NOS", "NOWO"] else "",
        "" if dados["ze_sem_ze"] else ssfid,
        dia,
        mes,
        ano,
    ...
    campos_6 = [
        ...
        dados["movel_3"],
        dados["movel_4"],
        dados["entry_tit_nome"],
        dados["entry_tit_conta"],
        dados["entry_tit_morada"],
        dados["entry_cp4"],
        dados["entry_cp3"],
        dados["entry_tit_contacto"],
        dados["entry_nif_antigo"],
        dados["entry_nif_novo"],
        dados["entry_tel_antigo"],
        dados["entry_tel_novo"],
        dados["entry_sim_antigo"],
        dados["entry_sim_novo"],
        dia,
        mes,
        ano,
        "" if dados["ze_sem_ze"] else ssfid,
    ...
    campos_7 = [
        ...
        mes,
        ano,
        dados["entry_res_localidade"].lower(),
    ]
    ...
    for (x, y), texto in zip(COORDS, campos):
        if texto:
            page.insert_text((x, y), str(texto), fontsize=11, color=(0,0,0))

    # pagina 4 do pdf
    for (x, y), texto in zip(COORDS_3, campos_3):
        if texto:
            page_3.insert_text((x, y), str(texto), fontsize=11, color=(0,0,0))

    # pagina 5 do pdf
    for (x, y), texto in zip(COORDS_4, campos_4):
        if texto:
            page_4.insert_text((x, y), str(texto), fontsize=11, color=(0,0,0))

    # pagina 6 do pdf
    for (x, y), texto in zip(COORDS_5, campos_5):
        if texto:
            page_5.insert_text((x, y), str(texto), fontsize=11, color=(0,0,0))

    # pagina 7 do pdf
    for (x, y), texto in zip(COORDS_6, campos_6):
        if texto:
            page_6.insert_text((x, y), str(texto), fontsize=11, color=(0,0,0))

    # pagina 8 do pdf
    for (x, y), texto in zip(COORDS_7, campos_7):
        if texto:
            page_7.insert_text((x, y), str(texto), fontsize=11, color=(0,0,0))
...
def gerar():
    ...
    print ( f"Res Móvel 1: {res_movel_1} | Res Móvel 2: {res_movel_2} | Res Móvel 3: {res_movel_3} | Res Móvel 4: {res_movel_4} " )



    fixo_1 = fixos[0].get().strip() if len(fixos) > 0 else ""
    fixo_2 = fixos[1].get().strip() if len(fixos) > 1 else ""

    # MÓVEIS
    movel_1 = telemoveis[0].get().strip() if len(telemoveis) > 0 else ""
    movel_2 = telemoveis[1].get().strip() if len(telemoveis) > 1 else ""
    movel_3 = telemoveis[2].get().strip() if len(telemoveis) > 2 else ""
    movel_4 = telemoveis[3].get().strip() if len(telemoveis) > 3 else ""
    print ( f"Fixo 1: {fixo_1} | Fixo 2: {fixo_2} | Móvel 1: {movel_1} | Móvel 2: {movel_2} | Móvel 3: {movel_3} | Móvel 4: {movel_4} " )
    time.sleep(2)  # Pequena pausa para evitar sobrecarga
    ...
    if operador != "MEO":
        for i, (entry_tel, entry_cvp) in enumerate(linhas_telemoveis):
            if i >= 4:
                break

            tel = entry_tel.get().strip()
            cvp = entry_cvp.get().strip()
            ...
            print(f"Telemóvel {i+1}: {tel} | CVP: {cvp}")
            time.sleep(2)  # Pequena pausa para evitar sobrecarga
    ...
    dados = {
        ...
        "nif_movel": entry_pm_nif.get().strip(),
        "combo_operador": combo_operador.get().strip(),
        "cvp_meo": entry_cvp_meo.get().strip(),
        "num_1": num_1,
        "cvp_1": cvp_1,
        "num_2": num_2,
        "cvp_2": cvp_2,
        ...
        "movel_3": movel_3,
        "movel_4": movel_4,
        "entry_tit_morada": entry_tit_morada.get().strip(),
        "entry_cp4": entry_cp4.get().strip(),
        "entry_cp3": entry_cp3.get().strip(),
        "entry_tit_contacto": entry_tit_contacto.get().strip(),
        "entry_nif_antigo": entry_nif_antigo.get().strip(),
        "entry_nif_novo": entry_nif_novo.get().strip(),
        "entry_tel_antigo": entry_tel_antigo.get().strip(),
        "entry_tel_novo": entry_tel_novo.get().strip(),
        "entry_sim_antigo": entry_sim_antigo.get().strip(),
        "entry_sim_novo": entry_sim_novo.get().strip(),
        "entry_tit_nome": entry_tit_nome.get().strip(),
        "entry_tit_conta": entry_tit_conta.get().strip(),
        "res_movel_1": res_movel_1, # RESCISAO
...
# global entry_res_nome, entry_res_morada, entry_res_cp4, entry_res_cp3, entry_res_localidade, combo_operadora_res, entry_res_cliente, entry_res_fixo
...
def abrir_interface(nome, ssfid):
    ...
    global entry_pm_nome, entry_pm_nif, entry_cvp_meo, linhas_telemoveis
    ...
    def copiar_dados_fa():
        ...
        for i, numero in enumerate(telefones_fa):
            if numero.strip():
                ...
                print("telemovel:", linhas_telemoveis[i][0].get())
    ...
    # Guardar linhas
    linhas_telemoveis = []
    print("Linhas telemóveis inicializadas:", linhas_telemoveis)
    ...
    global entry_tit_morada, entry_cp4, entry_cp3, entry_tit_contacto, entry_nif_antigo, entry_nif_novo, entry_tel_antigo, entry_tel_novo, entry_sim_antigo, entry_sim_novo
